...
const Menu = ({fixedMenu = false, cartToOpen, bgMenu}) => {
    ...
    if (fixedMenu) {
        return (
            <div className={`menu__header ${isVisible ? 'menu__show' : 'menu__hide'}`} style={{ background: bgMenu || 'transparent' }}>
                <InnerMenu cartToOpen={cartToOpen} />
            </div>
        )
    }
   ...
   { 
        return (
            <div className={`menu__header`} style={{ background: bgMenu || 'transparent' }}>
                <InnerMenu cartToOpen={cartToOpen} />
            </div>
        )
    }
...
const InnerMenu = ({cartToOpen}) => {
    return (
        <div className="menu__container">
            <div className="menu__inner d-flex justify-content-between w-100">
